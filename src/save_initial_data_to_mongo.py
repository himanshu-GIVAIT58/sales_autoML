...
# Map your local files (CSV or JSON) to MongoDB collection names
DATA_FILES = {
    ...
    "mongo_data.json": "all_collections",  # Example JSON file for all collections
}
    ...
    try:
        ...
        # Process JSON files
            ...
            with open(file_path, "r", encoding="utf-8") as json_file:
                ...
                if isinstance(data, list) and data:  # Check if it's a list of documents
                    collection.insert_many(data)
                    print(f"Inserted {len(data)} records into '{collection_name}' collection.")
                elif isinstance(data, dict):  # Handle JSON with multiple collections
