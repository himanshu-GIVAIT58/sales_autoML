...
from src.utils.cookie_utils import (
    ...
    create_auth_cookie_manager
)

load_dotenv()

# Initialize Keycloak client and cookie manager
...
def get_auth_url():
    ...
    return keycloak_client.get_auth_url(
...
def handle_callback():
    ...
    if 'code' in query_params and 'state' in query_params:
        ...
        # Use Keycloak client to authenticate
        auth_result = keycloak_client.authenticate_user(code)
        
        if auth_result:
            # Store in session
            st.session_state['authenticated'] = True
            st.session_state['user_info'] = auth_result['user_info']
            st.session_state['token_info'] = auth_result['token_info']
            
            # Save to cookies using utility function
            success, message = save_auth_to_cookies(auth_result['user_info'], auth_result['token_info'])
            if success:
                st.success(message)
            else:
                st.error(message)
...
def refresh_token():
    ...
    if 'token_info' in st.session_state and 'refresh_token' in st.session_state['token_info']:
        new_token_info = keycloak_client.refresh_token(
        ...
        if new_token_info:
            ...
            success, message = save_auth_to_cookies(st.session_state['user_info'], new_token_info)
            
            if success:
                st.success("Token refreshed successfully!")
            else:
                st.error(f"Failed to save refreshed token: {message}")
            return True
        else:
            st.error("Token refresh failed. Please login again.")
            logout()
            return False
...
def logout():
    ...
    if 'token_info' in st.session_state:
        ...
        keycloak_client.logout(
            st.session_state['token_info'].get('refresh_token', '')
        )
    
    # Clear session
    for key in ['authenticated', 'user_info', 'token_info']:
        if key in st.session_state:
            del st.session_state[key]
    
    # Clear cookies using utility function
    success, message = clear_auth_cookies()
    if not success:
        st.error(f"Failed to clear cookies: {message}")
...
def check_authentication():
    ...
    # Restore auth from cookies on first load using utility function
    if not st.session_state.get('auth_restored', False):
        success, auth_data = restore_auth_from_cookies()
        if success and auth_data:
            # Restore session state
            st.session_state['authenticated'] = True
            st.session_state['user_info'] = auth_data.get('user_info', {})
            st.session_state['token_info'] = auth_data.get('token_info', {})
        elif not success and "expired" not in str(auth_data).lower():
            st.error(f"Failed to restore auth: {auth_data}")
    ...
    if st.session_state.get('authenticated', False) and 'token_info' in st.session_state:
        token_info = st.session_state['token_info']
        if not keycloak_client.is_token_valid(token_info):
...
if not check_authentication():
    st.set_page_config(
        page_title="Login - Inventory Dashboard",
        page_icon="🔐",
        layout="centered"
    )
    
    st.title("🔐 Authentication Required")
    st.write("Please authenticate to access the Inventory Dashboard.")
    ...
    [🔐 Login with Keycloak]({auth_url})
    
    **Note:** Authentication state will be stored in secure cookies and persist across page refreshes.
    """)
    
    if st.button("Login with Keycloak", type="primary"):
    ...
    with st.expander("Debug Information"):
        ...
        # Show cookie info
        st.subheader("Cookie Status")
        try:
            all_cookies = cookie_manager.get_all_cookies()
            st.write("All Cookies:", all_cookies)
            
            auth_data = get_auth_cookie_data()
            if auth_data:
                st.write("Auth Cookie Data:", auth_data)
            else:
                st.write("No auth cookie found")
        except Exception as e:
            st.error(f"Error accessing cookies: {str(e)}")
    
    # Cookie manager component (required for functionality)
...
with st.sidebar:
    ...
    # Debug section for authenticated users
    with st.expander("🔧 Debug Info"):
        st.write("**User Info:**")
        st.json(user_info)
        st.write("**Token Expires:**")
        if 'token_info' in st.session_state and 'expires_at' in st.session_state['token_info']:
            expires_at = st.session_state['token_info']['expires_at']
            expires_date = datetime.fromtimestamp(expires_at / 1000)
            st.write(f"Token expires: {expires_date}")
        else:
            st.write("No expiration info")
